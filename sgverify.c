...
int main(int ac, char **av)
{
	...
	while (*++av) { 
		...
		for (k = 0; k < REPEAT; k++) { 
			...
			int ov = 0;
			size = SNAPPYC_FUNCTION_NAME_PREFIXsnappy_max_compressed_length(st_size);
			while (size > 0 && ov < N - 1) {
				size_t len = rand_seq() % size + 1;
				if (len > size)
					len = size;
			...
			assert (sum_iov(out_iov, ov) == 
				SNAPPYC_FUNCTION_NAME_PREFIXsnappy_max_compressed_length(st_size));

			size_t outlen_linear;
			char *out_linear = malloc(SNAPPYC_FUNCTION_NAME_PREFIXsnappy_max_compressed_length(st_size));
			err = SNAPPYC_FUNCTION_NAME_PREFIXsnappy_compress(&env, map, st_size, out_linear, &outlen_linear);
			if (err < 0)
